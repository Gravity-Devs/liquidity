...
echo $MNEMONIC_1 | $BINARY keys add validator --recover --keyring-backend=test --home $CHAIN_DIR/$CHAIN_ID
echo $MNEMONIC_2 | $BINARY keys add user1 --recover --keyring-backend=test --home $CHAIN_DIR/$CHAIN_ID 
echo $MNEMONIC_3 | $BINARY keys add user2 --recover --keyring-backend=test --home $CHAIN_DIR/$CHAIN_ID
$BINARY genesis add-genesis-account $($BINARY keys show validator --keyring-backend test -a --home $CHAIN_DIR/$CHAIN_ID) $GENESIS_COINS --home $CHAIN_DIR/$CHAIN_ID 
$BINARY genesis add-genesis-account $($BINARY keys show user1 --keyring-backend test -a --home $CHAIN_DIR/$CHAIN_ID ) $GENESIS_COINS --home $CHAIN_DIR/$CHAIN_ID
$BINARY genesis add-genesis-account $($BINARY keys show user2 --keyring-backend test -a --home $CHAIN_DIR/$CHAIN_ID ) $GENESIS_COINS --home $CHAIN_DIR/$CHAIN_ID 

echo "Creating and collecting gentx..."
$BINARY genesis gentx validator 1000000000stake --chain-id $CHAIN_ID --keyring-backend test --home $CHAIN_DIR/$CHAIN_ID 
$BINARY genesis collect-gentxs --home $CHAIN_DIR/$CHAIN_ID 
